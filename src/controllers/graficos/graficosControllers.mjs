...
const despesaPorTag = (req, res) => {
    ...
    graficosModel.despesaPorTag(casal, usuario, mes, ano, parceiro, (err, results) => {
        if (err) {
            return res.status(500).json(`Não foi possível encontrar os saldos das tags. ${err}` );
        }
...
export default { receitaPorCategoria, despesaPorCategoria, despesaPorTag }